...
def ExtractSmall(data, W, H, N):
    ...
    """
    pic = np.zeros((W,H))
    newSet = np.zeros((N*N, len(data[0,:])*W*H//(N*N)))     # Create an empty data set that holds data from all NxN squares.
    for k in range(0, len(data[0,:])):
        pic = data[:,k].reshape(H, W)    # Extract the k-th picture from the data set
        for i in range(0, H//N):
            for j in range(0, W//N):
                newSet[:, k*W*H//(N*N)+j+i*W//N] = pic[j*N:(j+1)*N, i*N:(i+1)*N].reshape(N*N)    # Extract the (i,j)-th NxN square from the k-th picture.
    return newSet
...
def MergeSmall(newSet, W, H, N):
    ...
    """
    data_len=len(newSet[0,:])//((H*W)//(N*N))
    newPic = np.zeros((data_len, H, W))     # Create data set of blank pictures with size WxH
    for k in range(0, data_len):
        for i in range(0, H//N):
            for j in range(0, W//N):
                newPic[k, j*N:(j+1)*N, i*N:(i+1)*N] = newSet[:, j+i*H//N+k*W*H//(N*N)].reshape(N,N)  # Add (i,j)-th NxN square to k-th picture.
    return newPic
...
if __name__ == "__main__":
    ...
    smallSet = ExtractSmall(data.T, W, H, N)  # Create a set of small NxN squares from data set
