...
@author: Sebastian
Using Amdahls law to determine the max number of processors 
"""
...
#    N_timer.log(0,log_s='20 data test, 14/03',open_file=1)
    ...
    for i in range(maxitr):
        ...
        t_seq_1 = time.time()-t_seq_0
        
        for q in range(qd):
            # print("q: {}".format(q))
            D_new = np.add(D_new, np.reshape(D_new_temp[q, :, :], (M, K)))
        t_par = time.time()-t_par_0 # execution time for the parallel part
        t_seq_2 = time.time()
...
#    N_timer.log("end",open_file=-1)
        t_seq = time.time()-t_seq_2+t_seq_1 # execution time for the sequential part
        t_total = t_seq+t_par
        print("t_seq: {}, t_par: {}, t_total: {}".format(t_seq_1,t_par,t_total))
    return D_old
