...
def itkrm(smallSet, K, S, maxit, nbr_threads, startD=np.array([1])):
    ...
    #maxit = 20
    threads = nbr_threads
    t_par_setup_0 = time.time()
    pool = mp.Pool(processes = threads)
    t_par_setup_1 = time.time()-t_par_setup_0
    #t_par = 0
    t_par = t_par_setup_1
...
#        start_time = N_timer.cont_timer(0, 0)
        ...
        t_par_0 = time.time()
        R = pool.map_async(_f, [(Y[:,n*N//(4*threads):(n+1)*N//(4*threads)], K, S, maxit, D_old, I_D[:,n*N//(4*threads):(n+1)*N//(4*threads)], n) for n in range((4*threads))]).get()
