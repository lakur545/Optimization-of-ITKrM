...
#    testSet = ImportImages.ExtractSmall(test_data.T, W_data, H_data, N_subpic)
    ...
    cuda.profile_start()
    t0 = time.time()
    D = gpu_itkrm(smallSet, K, S, maxit)
    dt = time.time() - t0
    cuda.profile_stop()
    print("\n{}".format(dt))
...
#        matproj = np.repeat(np.array([ D[:,I_D[:,n]] @ np.linalg.inv(DtD[I_D[:,n,None], I_D[:,n]]) @ DtY ]).T, S, axis=1)
#    dt = time.time()-t0
#    print(dt)
...
#    Y2 = Y.T.reshape(-1)*1
#    vecproj2 = np.zeros((N, M, S)).reshape(-1)*1
#    
#    TpB = 1
#    BpG = 1#math.ceil(N/TpB)
#    D_kernel[TpB, BpG](M, N, K, S, D2, DtD2, I_D2, Y2, vecproj2)
...
#    L = np.zeros((N, S, S)).reshape(-1)*1
#    U = np.zeros((N, S, S)).reshape(-1)*1
#    z = np.zeros(N*S)
#    x = np.zeros(N*S)
#    matproj3 = np.zeros((N, M)).reshape(-1)*1
##    test = np.zeros(S*N*M)
#
#    TpB = 32    # ThreadsPerBlock
#    BpG = math.ceil(N/32)     # BlocksPerGrid
#    t0 = time.time()
##    cuda.profile_start()
#    D_kernel2[TpB, BpG](M, N, K, S, D3, DtD3, I_D3, DtY3, Y3, vecproj3, L, U, z, x, matproj3)
##    cuda.profile_stop()
#    dt = time.time()-t0
#    print(dt)

#    D_CPU = itkrm.itkrm(smallSet, K, S, maxit)
#    np.random.seed(0)
#    D_GPU, I_D = gpu_itkrm(smallSet, K, S, maxit)
