...
def ineq_constraint1(x,Y,D,i):
    return -np.linalg.norm(Y[:,i]-np.dot(D,x),ord=2) + noise  # st. ||y-Dx||_2 < epsilon 
...
for i in range(itr):
    ...
    sol = fmin_slsqp(objective,x0,ieqcons=[lambda x: ineq_constraint1(x,Y,D,i)],iter=maxiter,iprint=0)    # using SciPy minimization solver SLSQP
    
    nonzero=np.sum(np.abs(sol)>zero_threshold)                                      # figuring how many intries are below our zero_threshold
    
    nnz_ar[i]=nonzero                                                                    # saving amount of values below zero threshold
    
    x_ar[:,i]=sol                                                                  # saving the found solution
...
Y_estimate=np.dot(D,x_ar)                                   # calculating the estimate of Y using our dictionary and the sparse X
